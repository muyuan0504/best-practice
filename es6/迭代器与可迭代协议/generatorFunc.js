aboutGenerateFn() // 关于生成器函数
// effectFn() // 生成器函数的副作用
// useYield() // yield的奇妙使用
// generatorInIterator() // 生成器混入可迭代对象

function aboutGenerateFn() {
    /**
     * 生成器函数
     * 生成器的定义方式是先创建一个函数，******调用这个函数再返回生成器对象g******
     * 这个g是一个可迭代对象(内部实现了Symbol.iterator)，可以通过Array.from(g),[...g],或for...of循环来使用它；
     *
    ...
    console.log(gFn.next())
    console.log(gFn.return(2)) // {value: 2, done: true}
    console.log(gFn.next()) // {value: undefined, done: true}

    /**
     * 对于内部有return的生成器函数，无论是扩展操作符、Array.from还是for...of来迭代这个生成器，无论return语句放在什么位置，结果中都不会包含返回的value
     * 
     * 因为g.return或return语句返回给迭代器的结果中包含done:true这个标志，以表明该序列已经结束；即使返回的结果中包含一个序列中的value，但是上述方法都会忽略它。
     * 对生成器而言，return语句应该主要用于 '断路保护' 功能，而不是提供序列中的最后一个值。
     */
    function* numbers2() {
        yield 1
        yield 2
    ...
    const g2 = numbers2()
    console.log('g2: ', [...numbers2()], Array.from(numbers2())) // [1, 2]  [1, 2]
    console.log(g2.next()) // {value: 1, done: false}
    console.log(g2.next()) // {value: 2, done: false}
    console.log(g2.next()) // { value: 2, done: true }
...
function generatorInIterator() {
    ...
    const execFn = fibonacci()
    console.log(execFn.next())
    console.log(execFn.next())
    console.log(execFn.next())
    console.log(execFn.next())
    console.log(execFn.next())
    ...
    const sequence = fibonacciObj[Symbol.iterator]()
    ...
    // const newSequence = fibonacciGenerator[Symbol.iterator]()
    // console.log(newSequence.next())
    // console.log(newSequence.next())
    // console.log(newSequence.next())
    // console.log(newSequence.next())
    // console.log(newSequence.next())
    ...
    for (const val of fibonacciGenerator) {
        console.log('for of 循环', val)
        if (val > 20) {
            break
        }
    }
